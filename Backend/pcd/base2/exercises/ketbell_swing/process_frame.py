...
class ProcessFrame:
    ...
    def _get_state(self, shoulder_angle):
        ...
        if self.thresholds['ELBOW_SHOULDER_VERT']['NORMAL'][1] <= shoulder_angle <= self.thresholds['ELBOW_SHOULDER_VERT']['NORMAL'][0]:
            knee = 1
        elif self.thresholds['ELBOW_SHOULDER_VERT']['TRANS'][1] <= shoulder_angle <= self.thresholds['ELBOW_SHOULDER_VERT']['TRANS'][0]:
            knee = 2
        elif self.thresholds['ELBOW_SHOULDER_VERT']['PASS'][1] <= shoulder_angle <= self.thresholds['ELBOW_SHOULDER_VERT']['PASS'][0]:
            knee = 3
    ...
    def process(self, frame: np.array, pose):
        ...
        if keypoints.pose_landmarks:
            ...
            # Camera is aligned properly.
                ...
                shoulder_vertical_angle = find_angle(np.array([shldr_coord[0], 0]),elbow_coord,shldr_coord )
                ...
                print("shoulder:"+str(shoulder_vertical_angle))
                ...
                if current_state == 's1':
                    if  shoulder_vertical_angle > self.thresholds['ELBOW_SHOULDER_VERT']['NORMAL'][1] and \
                ...
                frame = self._show_feedback(frame, self.state_tracker['COUNT_FRAMES'], self.FEEDBACK_ID_MAP, self.state_tracker['RAISE_ARMS'])
            ...
            self.state_tracker['start_inactive_time_front'] = time.perf_counter()
        if self.reps == self.state_tracker['SWING_COUNT']:
            display_duration = 60  
